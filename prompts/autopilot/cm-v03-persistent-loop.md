...
Follow-up feature after #9-#12:
- https://github.com/qike-ms/context-manager/issues/13 refactor context-manager into interface + swappable implementations so Hermes/OpenClaw/OpenCode/Pi/Goose-style context managers can conform and be swapped into agent-dispatcher
...
- After #9-#12 are complete, do not write DONE yet: create/ensure an issue-first supervisor task for #13, then continue the same loop on #13.
- #13 should start with an implementation-facing technical design and review because it is an architecture refactor; then implement via worker slices.
- If one child issue completes, immediately select the next child/follow-up issue. Do not stop while #8 has open in-scope children or #13 is open.
...
2. Check `gh issue view 8..13` / PRs / git status / tests to determine current ground truth.
3. Pick the first open child/follow-up issue needing design/implementation/review/PR. Order: #9, #10, #11, #12, then #13.
4. Ensure issue-first supervisor task state exists for that issue:
   `PYTHONPATH=/Users/emma/git/swe-supervisor python -m swe_supervisor.cli --state-root .swe-supervisor worker-start --issue <issue> --parent-issue 8 --issue-url https://github.com/qike-ms/context-manager/issues/<issue> --worker cm-v03-supervisor-loop --objective ... --acceptance ...`
...
11. If #9-#13 are closed/done and parent #8 can be closed, write DONE exactly:
Done: context-manager v0.3 scope #9-#12 plus interface refactor #13 implemented/reviewed/tested with evidence.
Todo: none for scoped loop.
